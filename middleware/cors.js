...
const allowHeaders = 'Content-Type,Authorization,Content-Length,X-Requested-With, privatekey'
...
const corsOptionsRequests = (req, res) => {
  ...
  res.header('Access-Control-Allow-Origin', req.header('origin')) //header was Origin
...
const corsSimpleRequests = (req, res, next) => {
  res.setHeader('Access-Control-Allow-Origin', '*')
  res.header('Access-Control-Allow-Origin', req.header('origin')) //header was Orgin
  res.header('Access-Control-Allow-Methods', 'GET,PUT,POST,DELETE,OPTIONS')
  res.header('Access-Control-Allow-Headers', allowHeaders)
  res.header('Access-Control-Allow-Credentials', 'true')
  res.header('Access-Control-Max-Age', '3600')
